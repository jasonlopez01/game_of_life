...
//use objects to create/display board, and give the board obj methods to udpate, etc.
...
function build_board(row_len, col_len){
	...
	$("td").each(function(){
		...
		for(var z = row_index - 1; z <= row_index + 1; z++){
			for(var p = col_index - 1; p <= col_index + 1; p++){
				n_row = (z + rl) % rl //modulo wraps grid
				n_col = (p + cl) % cl
...
$("#dims_form").on('submit', function (e) {
    var r = Math.min(Math.max($("#inputRows").val(), 30), 50)
    var c = Math.min(Math.max($("#inputCols").val(), 50), 80)
    //clear out board
    $("#board tr").remove();
    build_board(r,c)
    e.preventDefault();
